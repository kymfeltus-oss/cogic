...
export default async function Page({params,searchParams}:{params:Promise<{hotelId:string}>;searchParams:Promise<{checkIn?:string;checkOut?:string}>}){
 ...
 {staying?<div className="mt-6 rounded-2xl border border-green-400/30 bg-green-400/10 p-5"><p className="font-black text-green-300">✓ YOU&apos;RE STAYING HERE</p><p className="mt-2 text-lg">Your reservation: {staying.check_in} – {staying.check_out}</p><a href="/travel" className="mt-3 inline-block underline">View COGIC Travel</a><p className="mt-3 text-sm text-white/60">Need another room? View availability below.</p></div>:null}
