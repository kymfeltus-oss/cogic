...
import {
  detectBroadcastProvider,
  resolveSafeIngestProfile,
} from "@/lib/owner/provider-config";
...
function laneFromProbe(
  ...
  return {
    provider: detectBroadcastProvider(hlsUrl),
...
export async function buildOwnerBroadcastSnapshot(
  ...
  const hlsUrl = skipHls ? null : feed.primary.hlsUrl;
  const activeLane =
  ...
  const ingest = resolveSafeIngestProfile(
    feed.primary.hlsUrl,
    feed.backup.hlsUrl,
    row?.is_live === true && manifestReachable,
  );
  const preflight = buildPreflightChecks({
    ...
    requestedMode:
      requestedMode && requestedMode !== "none" ? requestedMode : undefined,

    vmix,
    feed,
    provider: feed.primary.provider,
    ingest,
    vmixRequired: process.env.VMIX_REQUIRED === "true",
  });
