...
requireOk("load entitlements", entitlementError); requireOk("load products", productError);
...
const { data: hotelRows, error: hotelError } = await db.from("housing_hotels").select("id,name").eq("program_key", programKey); requireOk("load hotels", hotelError);
...
console.log(JSON.stringify({ products: products.length, entitlements: entitlements.length, assignments: Object.values(assignmentKeys).flat().length, hotels: hotels.length, housingBlocks: 7, accessZones: 3, productsPublished: 0, occurrencesCreated: 0, policiesPublished: 0, staffAssigned: 0 }, null, 2));
